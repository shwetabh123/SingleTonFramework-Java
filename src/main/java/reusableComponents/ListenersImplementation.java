...
public class ListenersImplementation implements ITestListener{
	...
	public void onTestFailure(ITestResult result) {
		 ...
		 * PropertiesOperations.getPropertyValueByKey("automatic_Issue_Creation_In_JIRA"
		 * ); if(automaticJIRAcreation.trim().equalsIgnoreCase("ON")) { String issueS =
		 * "Automation Test Failed - "+result.getMethod().getMethodName(); String issueD
		 * = "Test Data to be passed here."; String issueNumber = null; try {
		 * issueNumber = jiraOps.createJiraIssue("QDPM", issueS, issueD, "10000", "5",
		 * "QDPM", "SIT", "5f782c4b95fe8e0069705791"); } catch (Exception e1) {
		 * e1.printStackTrace(); } try { jiraOps.addAttachmentToJiraIssue(issueNumber,
		 * screenshotPath); } catch (Exception e) { e.printStackTrace(); } }
		 */
